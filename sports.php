...
<div class="advanced-filters">
    <form id="searchForm">
        <div class="form-group">
            <label for="name" class="form-label">Coach Name / Sports Centre Name</label>
            <input 
                type="text" 
                id="name" 
                name="name" 
                class="form-input" 
                placeholder="Enter coach or sports centre name"
            >
        </div><br>
        
        <div class="form-grid">
            <div class="form-group">
                <label for="profession_type" class="form-label">Coach Type</label>
                <select id="profession_type" name="profession_type" class="form-select">
                    <option value="">--Select type--</option>
                    <option value="sports_coach">Sports Coach</option>
                    <option value="sports_coaching_centre">Sports Coaching Centre</option>
                </select>
            </div>
            ...
            <!-- Sports type field - always visible for sports search -->
            <div class="form-group">
                <label for="sports_type" class="form-label">Sports Type</label>
                <select id="sports_type" name="sports_type" class="form-select">
    ...
    <script>
        ...
        document.getElementById('searchForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.error) {
                    ...
                    let searchType = 'Sports Coaches';
                    const professionType = formData.get('profession_type');
                    const sportsType = formData.get('sports_type');
                    const city = formData.get('city');
                    
                    if (professionType) {
                        searchType = professionType === 'sports_coach' ? 'Sports Coaches' : 'Sports Coaching Centres';
                    }
                    
                    if (sportsType) {
                        searchType = `${sportsType} ${searchType}`;
                    }
            ...
            .catch(error => {
                ...
                document.getElementById('results').innerHTML = `
                    <div class="error-message">
                        ...
                        <div class="error-text">Search failed. Please check your connection and try again.</div>
                    </div>
                `;
            })
            .finally(() => {
        ...
        // Add some visual feedback for the sports theme
        document.addEventListener('DOMContentLoaded', function() {
            const title = document.querySelector('.form-label');
            if (title) {
                // Add sports-themed styling or animations here if needed
            }
        });
    </script>  </div>   
        </div>
...
function displayResults(coaches, searchType = 'Sports Coaches') {
    const resultsDiv = document.getElementById('results');
    
    // Debug: Log the coaches data
    console.log('Coaches data:', coaches);
    console.log('Number of coaches:', coaches ? coaches.length : 'coaches is null/undefined');
    
    if (!coaches || coaches.length === 0) {
        resultsDiv.innerHTML = `
            <div class="no-results">
                <div class="no-results-title">No Sports Coaches Found</div>
                <div class="no-results-text">Try adjusting your search criteria for sports type, location, or coach type</div>
                <div class="no-results-suggestions">
                    <p>Suggestions:</p>
                    <ul>
                        <li>Try searching without selecting a specific sports type</li>
                        <li>Check if there are coaches in nearby locations</li>
                        <li>Search for both individual coaches and coaching centres</li>
                    </ul>
                </div>
            </div>
        `;
        ...
        <div class="results-section">
            <div class="results-header">
                <div class="results-title">
                    ${searchType}
                    <span class="results-count">${coaches.length}</span>
                </div>
            </div>
    ...
    coaches.forEach((coach, index) => {
        // Debug: Log each coach
        console.log(`Coach ${index}:`, coach);
        
        const rating = coach.rating || 0;
        const stars = '★'.repeat(Math.floor(rating)) + '☆'.repeat(5 - Math.floor(rating));
        const initials = coach.name ? coach.name.split(' ').map(n => n[0]).join('').toUpperCase() : 'NA';
        
        // Handle professions more defensively
        const professions = coach.professions || {};
        
        // Collect sports-specific data
        const professionTypes = [];
        const allSportsTypes = [];
        const genderPreferences = [];
        const daysPerWeek = [];
        
        // Process sports_coach
        if (professions.sports_coach) {
            professionTypes.push('Sports Coach');
            if (professions.sports_coach.sports_type) {
                allSportsTypes.push(professions.sports_coach.sports_type);
            }
            if (professions.sports_coach.gender) {
                genderPreferences.push(professions.sports_coach.gender);
            }
            if (professions.sports_coach.days_per_week) {
                daysPerWeek.push(professions.sports_coach.days_per_week);
            }
        }
        
        // Process sports_coaching_centre
        if (professions.sports_coaching_centre) {
            professionTypes.push('Sports Coaching Centre');
            if (professions.sports_coaching_centre.sports_type) {
                allSportsTypes.push(professions.sports_coaching_centre.sports_type);
            }
            if (professions.sports_coaching_centre.gender) {
                genderPreferences.push(professions.sports_coaching_centre.gender);
            }
            if (professions.sports_coaching_centre.days_per_week) {
                daysPerWeek.push(professions.sports_coaching_centre.days_per_week);
            }
        }
        
        const uniqueSportsTypes = [...new Set(allSportsTypes)];
        const uniqueGenderPreferences = [...new Set(genderPreferences)];
        const uniqueDaysPerWeek = [...new Set(daysPerWeek)];
        
        const professionType = professionTypes.length > 0 ? professionTypes.join(', ') : 'Sports Professional';
        const availableServices = coach.available_services || professionType;
        ...
        html += `
            <div class="teacher-profile">
                ...
                <div class="rating-section">
                    ...
                    <span class="rating-reviews">(${coach.rating_count || 0})</span>
                </div>
                ...
                <div class="profile-details">
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-content">${coach.experience || 'Not specified'}</span>
                    </div>
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-content">${coach.city || 'Not specified'}</span>
                    </div>
                </div>

                <div class="contact-wrapper">
                    <a href="teacher-profile.php?id=${coach.id || coach.coach_id || encodeURIComponent(coach.name || 'unknown')}" class="contact-button">Check Profile</a>
                </div>
            </div>
        `;
    });
